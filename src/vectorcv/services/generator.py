import random
import spacy
from vectorcv.config import SPACY_MODEL
...
class RuleBasedBulletGenerator:
    ...
    def generate_tailored_bullets(self, missing_keywords: list[str], max_bullets: int = 4) -> list[str]:
        if not missing_keywords:
            return ["Resume already demonstrates strong alignment with target job requirements!"]

        bullets = []
        for kw in missing_keywords[:max_bullets]:
            category = self._classify_keyword(kw)
            template = random.choice(self.verb_templates if category == "verb" else self.noun_templates)
